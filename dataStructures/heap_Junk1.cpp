...
class Heap
{
	private:
		...
		DL_List<T> myQueue;
		void fixMaxViolation (int parent);
		void fixMinViolation (int parent);
...
// 
template<typename T>
void Heap<T>::fixMaxViolation(int parent)
{
...
// 
template<typename T>
void Heap<T>::fixMinViolation(int child)
{
	// parent = n; children = 2n+1, 2n+2
	// child = N; parent = -1 + ceil(n/2)
